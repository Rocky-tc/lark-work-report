...
- 完整 URL 在同一报告出现两次以上时使用 `W1` / `U1` Markdown 引用，正文写 `[W1][W1]`，定义只在“来源与覆盖”出现一次；只出现一次的来源继续使用行内链接，避免来源索引反而变长。
...
- 覆盖范围、快照、覆盖域、权限缺口和 work/uncertain 计数渲染为一行，但字段和值均不删除。
- 这些规则位于确定性渲染层，对内置格式和个人模板同时生效，不改变报告模型、证据覆盖或六段语义。
...
标准阶段接口新建模型 v5。它只包含账本指纹、五类叙事数组和 `wN` / `uN` 短引用：
...
`stage-io` 先把短引用展开为真实 `evidence_ids`，再从运行计划、主体、提取审计和完整账本确定性回填 `profile`、标题、覆盖信息、优先级、来源、排序依据与行动事实。下面的 v3 完整示例用于说明最终渲染输入和旧模型兼容格式。
...
- v5 每个非空条目必须提供一个或多个唯一 `evidence_refs`。摘要、进展、风险和下一周期只允许 `wN`；待复核只允许 `uN`。账本指纹不匹配、短引用越界或跨分区时拒绝定稿。
