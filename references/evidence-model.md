...
双账本核验完成后，`stage-io` 生成不含机器长标识和来源链接的综合包。Agent 用 `wN` / `uN` 短引用建立临时报告模型 v5；接口再把它们展开为账本 `cluster_id`。每个账本聚合项至少被一个相同类型的报告条目覆盖。模型通常只写叙事字段；单证据、无冲突且字段充分的项会由接口声明可直出的章节，模型可只交引用。主体、周期、覆盖信息、来源、排序依据和行动事实由接口从受管状态回填。报告模型不是新的证据层，也不得新增账本中不存在的事实。完整字段契约见 [report-rendering.md](report-rendering.md)。
