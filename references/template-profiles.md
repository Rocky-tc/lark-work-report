...
2. 只提取标题结构、五个正文语义槽的显示名称、列表或段落形式、工作流分组、字段标签，以及受限枚举形式的语气、人称和密度。
...
固定正文顺序为 `summary`、`progress`、`risks`、`next`、`uncertain`。样例没有“待复核”时使用内置名称补齐，不能删除。旧版档案中的 `coverage` 槽仍可读取，但不再显示或写入新的规范化档案。
...
不含周期词的中性名称可在三种报告中复用。每种报告的五个显示名称必须唯一。
...
智能体按档案中的 `tone` 生成报告模型措辞；定稿器负责标题、章节显示名称、列表形式、字段标签和工作流分组。模板不改变证据分类、来源锚、五槽顺序、内部覆盖验收及“结果、影响、决策、进展”的信息顺序。
