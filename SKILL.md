...
description: Use when 用户要基于飞书生成、整理或补全个人工作日报、周报、月报、指定周期工作总结，或要求复用已有日周报、排除闲聊和私人材料、保留可核验来源。
---
...
| `output` | 新建飞书文档；可选仅对话草稿 |
| `lark_profile` | 当前环境中已完成认证的 `lark-cli` profile |
| `evidence_retention` | `ephemeral` |
| `persistent_scope` | `work_and_uncertain`；仅持久化模式生效 |

日期有歧义时，先向用户展示解析后的北京时间范围。不要静默猜测跨周、跨月或跨年边界。

下文的 `<skill-dir>` 表示本 Skill 的安装目录，`<lark-profile>` 表示调用者配置并完成认证的 `lark-cli` profile。不得把示例占位符原样传给命令。
...
2. **确定策略。** 完整阅读 [report-profiles.md](references/report-profiles.md) 和 [collection-policy.md](references/collection-policy.md)。确认常规预算、硬上限、报告文档缓存与深挖上限。“尽量找全”仍使用 `standard`；只有用户明确说“deep / 深度报告 / 完整来龙去脉”并获知成本时才用 `deep`。

3. **只读采集。** 根据涉及的数据域，先读取对应的 `lark-calendar`、`lark-im`、`lark-doc`、`lark-wiki`、`lark-base`、`lark-task`、`lark-minutes` 或 `lark-vc` Skill。每条飞书命令使用：
...
8. **创建并回读。** 默认用 `lark-doc` 新建飞书文档，再执行 fetch 回读标题、章节、来源锚和覆盖说明。现有飞书内容保持只读。用户只要对话草稿时，跳过这两次调用。

9. **执行保留策略。** 文档回读成功后，`ephemeral` 模式清理本次候选池和分类上下文；`persistent` 模式写入已向用户展示位置的个人隔离证据包。报告缓存只指已验证的飞书报告文档，不创建本地报告缓存；它与证据包始终分开。
