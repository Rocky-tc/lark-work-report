...
- `prepare-run` 生成静态、可审阅并受摘要保护的执行图，同时只向 Agent 返回执行所需的轻量动作视图；运行时按图驱动现有节点，固定批量提取和一次综合，不引入新的语义阶段。
- 抓取与提取 Schema 复用共同定义，正文提取按精确去重后的真实模型包体分组；接口仍逐项校验唯一覆盖，不放宽准确性约束。
- 采集、提取和综合阶段分别使用 `iN`、`cN`、`wN` / `uN` 短引用；全局 ID、真实来源和文件路径留在受管私有回执中。
- 包 ID、输入摘要和账本指纹由私有回执注入；宿主可使用每包生成的 JSON Schema 约束语义结果。
...
- 支持宿主把 provider、model、节点用量和任务聚合用量写入独立文件，并对两个完整运行执行同数据、同模型、同质量签名的 A/B 比较。
