...
- 内置快照感知的周期解析、适配器能力协商、批量/并行编排、跨域去重、证据归并、状态裁决、重要性排序、正文队列过滤、接口预算、六段式确定性渲染、临时清理和报告校验脚本。
...
- 编译失败时生成只包含受影响候选的增量修复队列，不重新处理已验证项。
- 以 `stage-io` 作为 Agent 的两入口阶段接口；每轮只加载当前阶段包，陈旧结果或中途变化会被摘要校验拒绝。
- 采集、提取和综合阶段分别使用 `iN`、`cN`、`wN` / `uN` 短引用；全局 ID、真实来源和文件路径留在受管私有回执中。
- 报告模型 v5 省略主体、标题、周期、覆盖统计和可由账本推导的事实，由本地接口确定性回填。
- 重复来源自动改用 `W1` / `U1` 引用并只保存一次完整 URL；单次来源继续使用行内链接。
- 摘要中的可选影响或决策若已在详细章节逐字出现，只渲染一次；核心结果始终保留。
- 时间范围、快照、覆盖域、权限缺口和账本计数合并成一条完整覆盖说明。
...
输出统一为六段语义的紧凑报告：摘要、进展与结果、风险、下一周期重点、待复核、来源与覆盖。个人模板可以改变显示名称和文本布局，但不能删除或重排语义槽。标准流程的报告模型 v5 只保留叙事字段、账本指纹与短引用；本地阶段接口回填主体、周期、覆盖信息、真实证据 ID、多来源、排序依据、截止时间、待回复、行动类型和责任关系，再按固定顺序渲染。
...
- Agent 每次只读取 `stage-io` 生成的当前阶段包；包 ID、输入摘要和私有回执防止旧结果写回已经变化的运行状态。
- 模型只提交短引用与语义字段；稳定身份、来源、周期和覆盖信息由本地状态回填，重复或伪造这些字段会被拒绝。
